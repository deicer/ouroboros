...
0. Никогда не запускай `opencode`/`opencode run` через `run_shell` для правок кода.
   Для изменений в репозитории используй только `opencode_edit`.
   `run_shell` — только для проверок окружения/тестов.
1. Сначала полагайся на встроенный bootstrap: `opencode_edit` сам пытается установить OpenCode CLI
   (если бинарь отсутствует) и валидирует `opencode --version`.
2. Проверь, что `/app/opencode.json` существует и имеет провайдер `opencode`.
3. Проверь, что в env контейнера есть `OPENCODE_API_KEY`.
4. `opencode_edit` принимает только `prompt` и `cwd`.
   Никогда не передавай аргумент `model` в `opencode_edit` — это schema error.
5. Выбор моделей делай через env/конфиг OpenCode (дефолт + `OUROBOROS_OPENCODE_FALLBACK_MODELS`),
   а не через аргументы tool.
6. Всегда передавай в `opencode_edit` непустой и конкретный `prompt`:
   - цель (что изменить),
   - точные файлы,
   - ограничения (что не трогать),
   - критерий готовности.
7. Если задача большая (например рефакторинг огромного файла), разбивай на части.
   Не давай один гигантский промпт «перепиши всё сразу».
8. Верифицируй прямой вызов OpenCode в контейнере:
   - `opencode --version`
   - `opencode models opencode`
   - `opencode run -m opencode/minimax-m2.5-free "Reply with exactly: OK" --format json`
9. Если нет ответа/ошибка, всегда делай fallback с явной моделью:
   - `opencode run -m opencode/minimax-m2.5-free "<короткий тест>" --format json`
   - `opencode run -m opencode/trinity-large-preview-free "<короткий тест>" --format json`
10. Если дефолтный `opencode run ...` возвращает Copilot 403, не используй Copilot defaults.
   Используй модели провайдера OpenCode явно (`opencode/...-free`) и укажи это в статусе.
11. Пустой вывод не считать успехом: в таком случае верни диагностику и следующий шаг.
12. Если timeout/ошибка повторяется 2 раза подряд:
    - прекрати повтор того же вызова,
    - уменьши шаг/промпт,
    - дай владельцу короткий отчёт: причина + следующий конкретный шаг.
12.1. Не ставь диагноз «проблема с файловой системой» без подтверждения в tool result.
...
13. Не требуй `docker build` по умолчанию: сначала попробуй runtime-восстановление.
    Build/rebuild запрашивай только если отсутствуют системные бинарники, которые нельзя доставить в рантайме.
14. Сообщения владельцу — только на русском, кратко и по делу.
    Не отправляй в чат внутреннюю телеметрию вида `Delegating to OpenCode CLI...`.

### Эффективный skill по работе с OpenCode (обязательный)

Используй этот алгоритм каждый раз перед `opencode_edit`:
...
3. Для каждого шага используй шаблон промпта:
   - Что изменить (конкретно),
   - Где изменить (точный путь файла),
   - Что не трогать,
   - Как проверить готовность (тест/grep/ожидаемый diff).
4. После каждого шага сразу верифицируй:
   - `git_diff` и/или целевой тест перед следующим шагом.
5. Если ответ `No changes to apply`:
   - считать шаг выполненным, но всё равно проверить diff/тест.
6. Если `Internal Server Error` или timeout:
   - уменьшить шаг ещё в 2 раза, переформулировать короче и повторить.
