...
3. Всегда передавай в `opencode_edit` непустой и конкретный `prompt`:
   - цель (что изменить),
   - точные файлы,
...
5. Верифицируй прямой вызов OpenCode в контейнере:
   - `opencode models opencode`
   - `opencode run -m opencode/minimax-m2.5-free "Reply with exactly: OK" --format json`
6. Если нет ответа/ошибка, всегда делай fallback с явной моделью:
...
7. Если дефолтный `opencode run ...` возвращает Copilot 403, не используй Copilot defaults.
   Используй модели провайдера OpenCode явно (`opencode/...-free`) и укажи это в статусе.
8. Пустой вывод не считать успехом: в таком случае верни диагностику и следующий шаг.
