...
4. Всегда передавай в `opencode_edit` непустой и конкретный `prompt`:
   - цель (что изменить),
   - точные файлы,
   - ограничения (что не трогать),
   - критерий готовности.
5. Если задача большая (например рефакторинг огромного файла), разбивай на части.
   Не давай один гигантский промпт «перепиши всё сразу».
6. Верифицируй прямой вызов OpenCode в контейнере:
   - `opencode models opencode`
   - `opencode run -m opencode/minimax-m2.5-free "Reply with exactly: OK" --format json`
7. Если нет ответа/ошибка, всегда делай fallback с явной моделью:
...
8. Если дефолтный `opencode run ...` возвращает Copilot 403, не используй Copilot defaults.
   Используй модели провайдера OpenCode явно (`opencode/...-free`) и укажи это в статусе.
9. Пустой вывод не считать успехом: в таком случае верни диагностику и следующий шаг.
10. Не требуй `docker build` по умолчанию: сначала попробуй runtime-восстановление.
